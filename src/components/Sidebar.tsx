...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Persistence of MD3 Theming state
  const [globalTheme, setGlobalTheme] = useState<'light' | 'dark-pro'>(() => {
    return (localStorage.getItem('open-canvas-theme') as 'light' | 'dark-pro') || 'dark-pro';
  });

  useEffect(() => {
    document.documentElement.setAttribute('data-theme', globalTheme);
    localStorage.setItem('open-canvas-theme', globalTheme);
  }, [globalTheme]);
    ...
    const updateStats = () => {
      try {
        ...
        nodesList.forEach((n: any) => {
          const contentVal = n instanceof Y.Map ? n.get('content') : n.content;
          if (contentVal) {
            totalWords += contentVal.trim().split(/\s+/).filter(Boolean).length;
            totalChars += contentVal.length;
          }
        });
        setStats({
  ...
  const analyzeUnlinkedThoughts = () => {
    try {
      const nodesMap = ydoc.getMap<any>('nodes');
      const nodesList = Array.from(nodesMap.values()).map((n: any) => {
        return n instanceof Y.Map ? n.toJSON() : n;
      });
  ...
  const handleAiSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (query.includes('unlinked') || query.includes('bridge') || query.includes('connect')) {
        reply = 'I can detect unlinked thought associations. Click "Scan Bridge" to automatically identify them!';
  ...
  const renderFlowGraph = () => {
    ...
    return (
      <div className="bg-black/20 border border-white/10 rounded-xl p-3 select-none">
        <div className="flex items-center justify-between text-xs mb-2 font-display">
          <span className="text-secondary">Live cognitive flow:</span>
    ...
    <div className="relative h-full flex z-20">
      ...
      <div
        className={`glass-panel h-full flex flex-col transition-all duration-300 ease-in-out border-y-0 border-l-0 rounded-none overflow-hidden ${
          isOpen ? 'w-72 p-4' : 'w-0 p-0 border-r-0'
        }`}
      ...
      >
        {isOpen && (
          <>
            ...
            <div className="flex bg-black/20 border border-white/10 rounded-lg p-0.5 mb-5 shrink-0 text-secondary">
              <button
                title="Canvas files"
                onClick={() => setActiveTab('files')}
                className={`flex-1 py-1.5 rounded flex justify-center cursor-pointer transition-colors ${activeTab === 'files' ? 'bg-teal-500/15 text-teal-400 font-extrabold' : 'hover:text-primary'}`}
              >
              ...
              <button
                ...
                className={`flex-1 py-1.5 rounded flex justify-center cursor-pointer transition-colors ${activeTab === 'plugins' ? 'bg-teal-500/15 text-teal-400 font-extrabold' : 'hover:text-primary'}`}
              >
              ...
              <button
                ...
                className={`flex-1 py-1.5 rounded flex justify-center cursor-pointer transition-colors ${activeTab === 'ai' ? 'bg-teal-500/15 text-teal-400 font-extrabold' : 'hover:text-primary'}`}
              >
              ...
              <button
                ...
                className={`flex-1 py-1.5 rounded flex justify-center cursor-pointer transition-colors ${activeTab === 'dao' ? 'bg-teal-500/15 text-teal-400 font-extrabold' : 'hover:text-primary'}`}
              >
            ...
            <div className="flex-grow flex flex-col overflow-y-auto pr-1">
              ...
              {activeTab === 'files' && (
                <div className="flex flex-col gap-5 h-full">
                  <div>
                    <div className="text-[10px] uppercase font-bold tracking-widest text-muted mb-2 font-display">
                        ...
                        <div
                          ...
                          className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-all ${
                            currentWorkspaceId === ws.id
                              ? 'bg-teal-500/15 border border-teal-500/35 text-teal-200 shadow-sm'
                              : 'hover:bg-hover border border-transparent text-secondary hover:text-primary'
                          }`}
                        >
                          <div className="flex items-center gap-2 w-[70%]">
                            <Folder className={`w-4 h-4 ${currentWorkspaceId === ws.id ? 'text-teal-400' : 'text-muted'}`} />
                            {editingId === ws.id ? (
                              <input
                                type="text"
                                value={editName}
                                ...
                                className="bg-black/60 border border-white/20 rounded px-1.5 py-0.5 text-xs text-primary outline-none w-full"
                                autoFocus
                              />
                            ) : (
                          ...
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            {editingId === ws.id ? (
                              ...
                              <button
                                ...
                                className="p-0.5 rounded text-muted hover:text-primary hover:bg-white/10"
                              >
                            ...
                            <button
                              ...
                              className="p-0.5 rounded text-muted hover:text-rose-400 hover:bg-rose-500/10"
                            >
                    ...
                    <form onSubmit={handleCreateWorkspace} className="flex gap-1.5">
                      <input
                        ...
                        className="flex-grow px-2.5 py-1.5 rounded-lg bg-black/40 border border-white/10 outline-none text-xs text-primary placeholder-muted focus:border-teal-500/50"
                      />
                      <button
                        type="submit"
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-grow btn-premium py-1.5 cursor-pointer"
                        title="Export JSON"
                      >
                        <Download className="w-4 h-4 text-teal-400" />
                        <span className="text-xs">Backup</span>
                      </button>
                      <label className="flex-grow btn-premium py-1.5 cursor-pointer text-center flex items-center justify-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="bg-black/40 border border-white/10 p-2.5 rounded-xl mb-3">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <div className="flex items-center gap-1 text-xs text-muted font-display font-semibold">
                        <Users className="w-3.5 h-3.5 text-teal-400" />
                        <span>Online collaborators ({activePeers.length}):</span>
                      </div>
                      {activePeers.length === 0 ? (
                        <div className="text-[11px] italic text-muted pl-1">
                        ...
                        activePeers.map((p) => (
                          <div key={p.id} className="flex items-center gap-2 bg-white/5 border border-white/5 px-2.5 py-1.5 rounded-lg">
                            ...
                            <span className="text-xs text-primary truncate font-display">{p.name}</span>
                            <Wifi className="w-4 h-4 text-teal-400 ml-auto shrink-0" />
                          </div>
                        ))
                      )}
                    </div>
              ...
              {activeTab === 'plugins' && (
                <div className="flex flex-col gap-4">
                  {/* Strict Theme Selector dropdown for MD3 light and dark-pro */}
                  <div>
                    <div className="text-[10px] uppercase font-bold tracking-widest text-muted mb-2 font-display">
                      Theming system
                    </div>
                    <div className="flex bg-black/20 border border-white/10 rounded-lg p-0.5 mb-3">
                      {(['dark-pro', 'light'] as const).map((t) => (
                        <button
                          key={t}
                          onClick={() => {
                            setGlobalTheme(t);
                          }}
                          className={`flex-1 py-1.5 rounded text-center cursor-pointer text-xs font-bold transition-all ${
                            globalTheme === t 
                              ? 'bg-teal-500/20 text-teal-300 border border-teal-500/30' 
                              : 'text-secondary hover:text-primary'
                          }`}
                        >
                          {t === 'dark-pro' ? 'Dark Pro' : 'Light'}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center justify-between bg-black/40 border border-white/10 p-2.5 rounded-xl cursor-pointer hover:bg-white/5 transition-all select-none">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-primary font-display">Minimalist focus</span>
                          <span className="text-[10px] text-muted">Hide grids, colors & shadows</span>
                        </div>
                        <input
                          type="checkbox"
                      ...
                      <label className="flex items-center justify-between bg-black/40 border border-white/10 p-2.5 rounded-xl cursor-pointer hover:bg-white/5 transition-all select-none">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-primary font-display">Late-night insomnia sepia</span>
                          <span className="text-[10px] text-muted">Warming blue-light contrast filter</span>
                        </div>
                        <input
                          type="checkbox"
                  ...
                  <div>
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-xl p-3 flex flex-col gap-2 font-display">
                      ...
                      <div className="flex justify-between text-xs">
                        <span className="text-secondary">Char count:</span>
              ...
              {activeTab === 'ai' && (
                <div className="flex flex-col gap-4 h-full">
                  <div>
                    ...
                    <div className="flex flex-col gap-2 bg-black/40 border border-white/10 rounded-xl p-3 h-[200px] overflow-y-auto mb-2">
                      {aiChat.map((msg, idx) => (
                        <div key={idx} className={`text-xs p-2 rounded-lg leading-relaxed ${msg.role === 'user' ? 'bg-indigo-500/15 text-indigo-200 ml-4 self-end' : 'bg-white/5 text-secondary mr-4 self-start'}`}>
                    ...
                    <form onSubmit={handleAiSubmit} className="flex gap-1.5">
                      <input
                        ...
                        className="flex-grow px-2 py-1.5 rounded-lg bg-black/40 border border-white/10 outline-none text-xs text-primary focus:border-indigo-500/50"
                      />
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button
                        ...
                        className="text-[10px] uppercase font-bold tracking-wider px-2 py-1 border border-teal-500/30 rounded bg-teal-500/10 hover:bg-teal-500/30 text-teal-300 cursor-pointer transition-colors"
                      >
                        Scan bridge
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5 max-h-[140px] overflow-y-auto pr-1">
                      {unlinkedAlerts.length === 0 ? (
                        <div className="text-[11px] text-muted italic pl-1">
                ...
                <div className="flex flex-col gap-4">
                  <div>
                    ...
                    <div className="flex flex-col gap-3">
                      {proposals.map((prop) => (
                        <div key={prop.id} className="bg-black/40 border border-white/10 rounded-xl p-3 flex flex-col gap-2 select-none">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-[9px] uppercase font-bold text-muted">Consensus active</span>
                          </div>
                          <p className="text-xs text-secondary leading-relaxed font-display">{prop.title}</p>
                          <div className="flex items-center justify-between text-[11px] font-mono mt-1 pt-2 border-t border-white/5 text-muted">
      ...
      <button
        ...
        className="absolute top-1/2 -translate-y-1/2 -right-3.5 glass-panel p-1 rounded-full border border-white/10 text-muted hover:text-primary transition-colors cursor-pointer z-30"
